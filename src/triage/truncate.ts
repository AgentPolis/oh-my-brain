...
export function truncateIfNeeded(content: string, contentType: ContentType): string {
  ...
  // Budget: 70% for head, 20% for tail (in chars, approximated as tokens * 4)
  const budgetChars = TOOL_OUTPUT_MAX_TOKENS * 4;
  const headChars = Math.floor(budgetChars * 0.70);
  const tailChars = Math.floor(budgetChars * 0.20);
